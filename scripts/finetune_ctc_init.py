#!/usr/bin/env python3
"""CTC-only adaptation of IndicConformer hybrid. --init lets us start from a grafted .nemo
(e.g. base+SSL-encoder for v9b). Otherwise identical recipe to the v5 finetune."""
...
def main():
    ...
    m=nemo_asr.models.EncDecHybridRNNTCTCBPEModel.restore_from(a.init, map_location="cuda")
    ...
    m.training_step=types.MethodType(ctc_training_step, m)
    ...
    trainer=pl.Trainer(
        ...
        limit_val_batches=0.0, num_sanity_val_steps=0,
    ...
    print(f"[finetune] init={a.init.split('/')[-1]} lr={a.lr} bs={a.bs} epochs={a.epochs}", flush=True)
    trainer.fit(m)
    if not a.smoke:
